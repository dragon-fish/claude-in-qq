...
`skills/` 下每个目录都是一个 skill，装的时候全部软链过去，加新的不必改脚本。除 `qq-notify` 外还有
`patch-cc-in-qq`——写给**要改这个仓库本身**的 Claude，只讲源码里读不到的事：重启时怎么不把正在对话的
自己弄断线、QQ 的接口文档该去哪找、没有测试框架时怎么验证。结构和分层不写进去，让它直接读源码更准，
也不会过期。
...
要改这个项目基本躲不开查接口，而 QQ 的文档有几个坑，知道了能省很多时间。

入口是 <https://bot.q.qq.com/wiki/develop/api-v2/>。它的左侧导航**按当前页展开**，抓索引页只能
拿到折叠后的顶层——想知道某一支下面有什么，得先打开那一支里任意一个已知页面。照着索引页断定
「文档里没有」多半是错的：指令面板在 `server-inter/menu-panel/`，流式消息在
`autogen/api/v2_users_user_openid_stream_messages.post.html`，两者都不出现在索引页的可见导航里。

按 URL 路径拼出来的自动生成参考都在 `autogen/api/` 下，比人工撰写的章节齐全。

官方还有个 markdown 文档源仓库 <https://github.com/tencent-connect/bot-docs>，克隆下来本地 grep
比 GitHub 代码搜索可靠（后者对它没有索引）。但它停更于 2025-04，指令面板、流式消息都不在里面。

**文档并不全，尤其是错误码。** 流式消息那页只列了 `40007` / `50001` / `50002`，而实际会收到：

| 错误码 | 含义 | 应对 |
| --- | --- | --- |
| `40034020` | 同一流式消息发送超过时间限制（观测到约 10 分钟） | 开一条新流接着写 |
| `40034019` | 流式引导已经结束 | **不要重开**，同一条 inbound 上再开也会被拒 |

两者的区别是前者针对单条流、后者针对整条 inbound 消息，混为一谈会在进程关闭时把回复剁成一串
两字气泡。文档里也没写被动回复的额度会限制重开次数（单聊每条 inbound 4 次、60 分钟内有效）。

遇到没记载的行为，从实际响应反推，然后把结论写进使用处的注释——下一个人同样查不到。
