...
**有限流**：5 分钟内相同内容、每小时 10 条、每天 50 条。这不是权限控制——能跑这个命令的进程本来就能
读到旁边的凭据直接调 QQ API，拦不住也没必要拦。它防的是失控的循环：主动消息 1000 条/天是**账号级**
额度，和 bridge 共用，被打光的后果不是你被骚扰，而是 bridge 在对话中途突然发不出消息，而你那头看不出
任何原因。
